    ...
    if (data !== undefined && data !== null) {
      ...
      if (typeof data === 'string') {
        ...
        switch (convertCaseOptions) {
          case 'snake_case':
            converted = convertCaseMap.snakeCase[data];
            if (!converted) {
              converted = toSnakeCase(data);
              convertCaseMap.snakeCase[data] = converted;
            }
            break;
          case 'kebab-case':
            converted = convertCaseMap.kebabCase[data];
            if (!converted) {
              converted = toKebabCase(data);
              convertCaseMap.kebabCase[data] = converted;
            }
            break;
          case 'camelCase':
            converted = convertCaseMap.camelCase[data];
            if (!converted) {
              converted = toCamelCase(data);
              convertCaseMap.camelCase[data] = converted;
            }
            break;
          default: // Do nothing
        }
