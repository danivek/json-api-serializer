...
module.exports = class JSONAPISerializer {
  ...
  processOptionsValues(data, extraData, options, fallbackModeIfOneArg) {
    ...
    if (options && typeof options === 'function') {
      // Support optional 'extraData' arguments
      if (options.length === 2) {
        processedOptions = options(data, extraData);
      } else { // Fallback mode
        processedOptions = fallbackModeIfOneArg === 'extraData' ? options(extraData) : options(data);
      }
    } else {
      Object.keys(options).forEach((key) => {
        ...
        if (options[key] && typeof options[key] === 'function') {
          ...
          if (options[key].length === 2) {
            processedValue = options[key](data, extraData);
          } else { // Fallback mode
            processedValue = fallbackModeIfOneArg === 'extraData' ? options[key](extraData) : options[key](data);
          }
        } else {
