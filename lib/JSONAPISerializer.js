...
module.exports = class JSONAPISerializer {
   ...
   */
  serializeData(type, data, options, included, extraData, dataRepository = {}) {
    // Empty data
    if (_.isEmpty(data)) {
      // Return [] or null
    ...
    if (Array.isArray(data)) {
      return _.uniqWith(data.map(d => this.serializeData(type, d, options, included, extraData, dataRepository)), _.isEqual);
    }

    const id = data[options.id] ? data[options.id].toString() : undefined;
    ...
    if (_.has(dataRepository, [type, id])) {
      _.merge(_.get(dataRepository, [type, id]), serializedData);
    } else {
      _.setWith(dataRepository, [type, id], serializedData, Object);
    }

    return _.get(dataRepository, [type, id]);
  }
   ...
   * @return {Object|Object[]} serialized data.
   */
  serializeMixedData(typeOption, data, included, extraData, dataRepository = {}) {
    // Empty data
    if (_.isEmpty(data)) {
      // Return [] or null
      return Array.isArray(data) ? data : null;
    ...
    // Array data
    if (Array.isArray(data)) {
      return _.uniqWith(data.map(d => this.serializeMixedData(typeOption, d, included, extraData, dataRepository)), _.isEqual);
    }
    ...
    return this.serializeData(type, data, this.schemas[type].default, included, extraData, dataRepository);
  }
