...
module.exports = class JSONAPISerializer {
  ...
  validateOptions(options) {
    ...
    Object.keys(relationships).forEach(key => {
      ...
      if (
        ...
        throw new Error(
          `option \`type\` for relationship \`${key}\` must be a string or a function`
        );
      }
      if (
        relationships[key].alternativeKey &&
        typeof relationships[key].alternativeKey !== 'string'
      ) {
      ...
      if (
        ...
        throw new Error(
          `option \`links\` for relationship \`${key}\` should be an object or a function`
        );
      }
      if (
        ...
        throw new Error(
          `option \`meta\` for relationship \`${key}\` should be an object or a function`
        );
      }
  ...
  validateDynamicTypeOptions(options) {
    ...
    if (
      ...
      throw new Error('option `topLevelLinks` should be an object or a function');
    if (
      options.topLevelMeta &&
      typeof options.topLevelMeta !== 'object' &&
      typeof options.topLevelMeta !== 'function'
    )
      throw new Error('option `topLevelMeta` should be an object or a function');
    if (typeof options.jsonapiObject !== 'boolean')
      throw new Error('option `jsonapiObject` should a Boolean');
  ...
  serialize(type, data, schema, extraData) {
    ...
    if (typeof type === 'object') {
      // Serialize data with the dynamic type
  ...
  deserialize(type, data, schema) {
    ...
    if (data.data) {
      if (Array.isArray(data.data)) {
        deserializedData = data.data.map(resource =>
  ...
  deserializeResource(type, data, schema, included) {
    ...
    if (data.relationships) {
      Object.keys(data.relationships).forEach(relationshipProperty => {
        ...
        if (
          resourceOpts.relationships[relationshipKey] &&
          resourceOpts.relationships[relationshipKey].alternativeKey
        ) {
        ...
        const deserializeFunction = relationshipData => {
          if (
            resourceOpts.relationships[relationshipKey] &&
            resourceOpts.relationships[relationshipProperty].deserialize
          ) {
              ...
              relationship.data.map(d =>
                included
                  ? this.deserializeIncluded(
                      d.type,
                      d.id,
                      resourceOpts.relationships[relationshipProperty],
                      included
                    )
  ...
  serializeRelationship(rType, rSchema, rData, included, data, extraData) {
    ...
    if (!isPlainObject(rData)) {
      ...
      included.set(
        ...
        this.serializeData(type, rData, rOptions, included, extraData)
      );
    }
