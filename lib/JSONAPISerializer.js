...
module.exports = class JSONAPISerializer {
   ...
   * @param {object} [extraSchemaOptions=] additional schema options, a map of types with options to override
   * @returns {object} serialized data.
   */
  serialize(type, data, schema, extraData, excludeData, extraSchemaOptions = {}) {
    ...
    if (extraSchemaOptions[type]) {
      // Merge default (registered) options and extra options into new options object
      options = Object.assign({}, options, extraSchemaOptions[type]);
    }
   ...
   */
  serializeAsync(type, data, schema, extraData, excludeData, extraSchemaOptions = {}) {
    // Support optional arguments (schema)
    if (arguments.length === 3) {
      if (typeof schema === 'object') {
        extraData = schema;
   ...
   * @returns {object|object[]} serialized data.
   */
  serializeMixedResource(typeOption, data, included, extraData, extraSchemaOptions = {}) {
    if (isEmpty(data)) {
      // Return [] or null
      return Array.isArray(data) ? data : null;
    }
    ...
    return this.serializeResource(type, data, options, included, extraData);
  }
