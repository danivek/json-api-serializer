  ...
  deserializeResource(type, data, schema, included) {
    ...
    if (data.relationships) {
      Object.keys(data.relationships).forEach((relationshipProperty) => {
        ...
        let deserializeFunction;
        if (resourceOpts.relationships[relationshipKey]) {
          if (resourceOpts.relationships[relationshipKey].alternativeKey) {
            relationshipKey = resourceOpts.relationships[relationshipKey].alternativeKey;
          }
          if (resourceOpts.relationships[relationshipProperty].deserialize) {
            deserializeFunction = resourceOpts.relationships[relationshipProperty].deserialize
          }
        }
        ...
        if (relationship.data !== undefined) {
          if (Array.isArray(relationship.data)) {
            ...
            _set(deserializedData, relationshipKey, relationship.data.map(d => (included
              ...
              : deserializeFunction ? deserializeFunction(d) : d.id)));
            ...
            _set(deserializedData, relationshipKey, included
              ...
              : deserializeFunction ? deserializeFunction(relationship.data) : relationship.data.id);
          }
        }
      });
    }
