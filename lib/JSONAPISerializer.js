...
module.exports = class JSONAPISerializer {
  ...
  serializeResource(type, data, options, included, extraData, overrideSchemaOptions = {}) {
    ...
    const { id, attributes } = serializeId(data, options);

    return {
      type,
      id,
      attributes: this.serializeAttributes(attributes, options),
  ...
  serializeRelationship(
    ...
    if (!isObjectLike(rData)) {
      ...
      const { id, attributes } = serializeId(rData, rOptions);
      serializedRelationship.id = id;

      const isIdOnly = Object.keys(attributes).length === 0;
