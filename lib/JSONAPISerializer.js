...
module.exports = class JSONAPISerializer {
    ...
    if (data.relationships) {
      Object.keys(data.relationships).forEach(relationshipProperty => {
        ...
        let relationshipKey = options.unconvertCase
          ? this._convertStringCase(relationshipProperty, options.unconvertCase)
    ...
    Object.keys(options.relationships).forEach(relationship => {
      ...
      if (
        ...
        relationship = options.convertCase
          ? this._convertStringCase(relationship, options.convertCase)
          : relationship;
      ...
      if (isPlainObject(data)) {
        return transform(
          ...
          (result, value, key) => {
            ...
            result[this._convertStringCase(key, convertCaseOptions)] = converted;
            return result;
          },
          {}
        );
  ...
  /**
   * Convert a string to differentcases
   *
   * @function JSONAPISerializer#_convertStringCase
   * @private
   * @param {string} data to convert
   * @param {string} convertCaseOptions can be snake_case', 'kebab-case' or 'camelCase' format.
   * @returns {string} Converted string
   */
  _convertStringCase(data, convertCaseOptions) {
    let converted;
