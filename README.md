...
* **relationships** (optional): An object defining some relationships
  * relationship: The property in data to use as a relationship
    ...
    * **data** (optional): A _function_ `function(relationshipData) { ... }` that receives the relationship's value (i.e. the property resolved from the relationship key or `alternativeKey`, _not_ the whole resource) and returns the data to serialize for the relationship. Useful to unwrap a nested shape, e.g. `data: (relationshipData) => relationshipData.data`.
